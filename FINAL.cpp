...
//II. DFS
void DFS(int pos, int table[30][30], char colors[30][30], int count[100], char country[100], int numOfColors, int v0[100],int h0[100]) {
	...
	if(table[v][h] == 0) {
		colors[v][h] = 96;
		DFS(pos + 1, table, colors, count, country, numOfColors, v0, h0);
	} else {
...
int main() {
	...
	int h0[100]; 
	//Scan the file
    freopen("color900_2.txt","r",stdin);
    for(i=0; i<30; i++) {
    	for(j=0; j<30; j++) {
    		scanf("%2d", &table[i][j]);
    	}
    ...
    for(ans = 0;ans == 0; ) {
	DFS(0, table, colors, count, country, numOfColors, v0, h0);
	numOfColors++;
	}
